<x-form-section submit="create_production_focal">
    <x-slot name="title">Focal Leadform</x-slot>

    <x-slot name="description">Focal Leadform for marking still images</x-slot>
    ...
    <x-slot name="form">
        ...
        <div class="col-span-6">
            <x-label :value="'Out of Stock Count'" for="oos_count" />

            <x-input wire:model="oos_count" class="block w-full mt-1" id="oos_count" name="oos_count" type="number" />
            <small class="dark:text-gray-400">You may put 0 or you can leave it as blank.</small>

            <x-input-error for="oos_count" class="mt-2" />
        </div>

        <div class="col-span-6">
            <x-label :value="'Not Out of Stock Count'" for="not_oos_count" />

            <x-input wire:model="not_oos_count" class="block w-full mt-1" id="not_oos_count" name="not_oos_count" type="number" />
            <small class="dark:text-gray-400">You may put 0 or you can leave it as blank.</small>

            <x-input-error for="not_oos_count" class="mt-2" />
        </div>

        <div class="col-span-6">
            <x-label :value="'Discard Count'" for="discard_count" />

            <x-input wire:model="discard_count" class="block w-full mt-1" id="discard_count" name="discard_count" type="number" />
            <small class="dark:text-gray-400">You may put 0 or you can leave it as blank.</small>

            <x-input-error for="discard_count" class="mt-2" />
        </div>

        <div class="col-span-6">
            <x-button wire.loading.attr="disabled">
                <span wire:loading.flex wire:target="create_production_focal">
                    <x-loading-indicator
                ...
                <span wire:loading.remove wire:target="create_production_focal">Submit</span>
            </x-button>
        </div>
    </x-slot>

    <x-slot name="actions">
        <x-action-message class="text-green-500" on="production-focal-created">Leadform for Focal successfully submitted!</x-action-message>
    </x-slot>
</x-form-section>
